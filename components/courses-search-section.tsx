...
export function CoursesSearchSection({
  ...
  const defaultCourses = courses.toSorted((a, b) => a.order - b.order);
  const groupedDefaultCourses = groupCoursesBySemester(defaultCourses);
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {activeSearchedCourses.map((course: CourseListItem) => (
              <Link
                key={course._id}
                href={`/${universitySlug}/${majorSlug}/${course.slug}`}
                className="group rounded-xl border border-surface-200 bg-white p-4 shadow-sm transition-all hover:border-primary-300 hover:shadow-md dark:border-surface-700 dark:bg-surface-900 dark:hover:border-primary-600"
              >
                <div className="flex items-center gap-2">
                  {course.semester ? (
                    <span className="rounded-md bg-primary-50 px-2 py-0.5 text-xs font-medium text-primary-700 dark:bg-primary-950 dark:text-primary-300">
                      {semesterLabels[course.semester] ||
                        `فصل ${course.semester}`}
                    </span>
                  ) : (
                    <span className="rounded-md bg-surface-100 px-2 py-0.5 text-xs font-medium text-surface-500 dark:bg-surface-800 dark:text-surface-400">
                      مادة عامة
                    </span>
                  )}
                </div>
                <h3 className="mt-3 truncate text-sm font-semibold text-surface-800 group-hover:text-primary-600 dark:text-surface-100 dark:group-hover:text-primary-400">
                  {course.name}
                </h3>
                {course.courseCode && (
