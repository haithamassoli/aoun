...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-1 flex-row gap-3 overflow-x-auto px-3 py-4 sm:gap-4 sm:px-4 sm:py-5 md:px-6 lg:gap-6 lg:px-8 lg:py-6">
      {/* Sidebar */}
      <aside className={`shrink-0 transition-[width] duration-200 ${isSidebarExpanded ? "w-64" : "w-20"}`}>
        <div className="rounded-2xl border border-surface-200 bg-white p-4 shadow-sm dark:border-surface-700 dark:bg-surface-900">
          <div className="mb-4 flex items-center justify-between border-b border-surface-100 pb-4 dark:border-surface-800">
            {isSidebarExpanded && <p className="text-sm font-semibold text-surface-900 dark:text-surface-50">لوحة التحكم</p>}
            <button
              type="button"
            ...
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                {isSidebarCollapsed ? (
                  ...
                  <path strokeLinecap="round" strokeLinejoin="round" d="M11 19l-7-7 7-7M19 5v14" />
                )}
              </svg>
            </button>
          </div>
          ...
          <nav className="space-y-1">
            ...
            {user.role === "admin" && (
              <div className="pt-2">
                {isSidebarExpanded && <p className="mb-2 px-3 text-xs font-semibold uppercase tracking-wider text-surface-400 dark:text-surface-500">الإدارة</p>}
                {[
            ...
            {/* Majors list */}
            <div className="pt-2">
              {isSidebarExpanded && <p className="mb-2 px-3 text-xs font-semibold uppercase tracking-wider text-surface-400 dark:text-surface-500">التخصصات</p>}
              {majors === undefined ? (
                <div className={`py-2 text-xs text-surface-400 dark:text-surface-500 ${isSidebarExpanded ? "px-3 text-right" : "text-center"}`}>
                ...
                majors.map((major: { _id: string; name: string }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isSidebarExpanded && <span className="truncate">{major.name}</span>}
                    </Link>
                  );
                })
              )}
