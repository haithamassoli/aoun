...
const EMPTY_FORM: FormData = { name: "", slug: "", logoUrl: "", order: "0", alias: "" };
...
export default function AdminUniversitiesPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleting(id);
    try {
      await removeUniversity({ token: sessionToken, universityId: id as Id<"universities"> });
  ...
  return (
    <div>
      ...
      <nav className="mb-4 flex items-center gap-2 text-sm text-surface-500 dark:text-surface-400">
        ...
        <span className="font-medium text-surface-900 dark:text-surface-50">الجامعات</span>
      </nav>

      {/* Header */}
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-surface-900 dark:text-surface-50">إدارة الجامعات</h1>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 rounded-2xl border border-primary-200 bg-primary-50/30 p-5 dark:border-primary-800 dark:bg-primary-950/30">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-xs font-medium text-surface-600 dark:text-surface-300">اسم الجامعة *</label>
              <input
                type="text"
                value={formData.name}
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-surface-600 dark:text-surface-300">رابط الشعار (URL)</label>
              <input
                type="url"
                dir="ltr"
      ...
      {universities === undefined ? (
        ...
        <div className="rounded-2xl border border-surface-200 bg-white p-12 text-center dark:border-surface-700 dark:bg-surface-900">
          <p className="text-sm font-medium text-surface-700 dark:text-surface-200">لا توجد جامعات</p>
          <p className="mt-1 text-xs text-surface-400 dark:text-surface-500">أضف جامعة جديدة للبدء</p>
        </div>
      ) : (
        <div className="space-y-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <h3 className="truncate text-sm font-semibold text-surface-900 dark:text-surface-50">{uni.name}</h3>
                    <div className="flex items-center gap-2 text-xs text-surface-500 dark:text-surface-400">
                      <span dir="ltr">{uni.slug}</span>
                      {uni.alias && <span className="rounded bg-surface-100 px-1.5 py-0.5 dark:bg-surface-800">{uni.alias}</span>}
                    </div>
                  </div>
                </div>
              </div>
