...
const EMPTY_FORM: UserFormData = { name: "", email: "", role: "contributor" };
...
export default function AdminUsersPage() {
  ...
  const handleEdit = (u: { _id: string; name: string; email: string; role: "admin" | "contributor" }) => {
  ...
  const handleAddPermission = async () => {
    ...
    try {
      ...
      const msg =
        error instanceof Error && error.message.includes("PERMISSION_ALREADY_EXISTS")
  ...
  const handleRemovePermission = async (permissionId: string) => {
    if (!sessionToken) return;
    try {
      await removePermission({ token: sessionToken, permissionId: permissionId as Id<"permissions"> });
  ...
  const assignedMajorIds = new Set(userPermissions?.map((p) => p.majorId) ?? []);
  const availableMajors = majors?.filter((m) => !assignedMajorIds.has(m._id)) ?? [];
  ...
  return (
    <div>
      ...
      <nav className="mb-4 flex items-center gap-2 text-sm text-surface-500 dark:text-surface-400">
        ...
        <span className="font-medium text-surface-900 dark:text-surface-50">المستخدمون</span>
      </nav>

      {/* Header */}
      <div className="mb-6">
        <h1 className="text-xl font-bold text-surface-900 dark:text-surface-50">إدارة المستخدمين</h1>
      ...
      {editingId && (
        <form onSubmit={handleSubmit} className="mb-6 rounded-2xl border border-primary-200 bg-primary-50/30 p-5 dark:border-primary-800 dark:bg-primary-950/30">
          <h3 className="mb-4 text-sm font-semibold text-surface-800 dark:text-surface-100">تعديل المستخدم</h3>
          <div className="grid gap-4 sm:grid-cols-3">
            <div>
              <label className="mb-1 block text-xs font-medium text-surface-600 dark:text-surface-300">الاسم *</label>
              <input
                type="text"
                value={formData.name}
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-surface-600 dark:text-surface-300">البريد الإلكتروني *</label>
              <input
                type="email"
                dir="ltr"
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-surface-600 dark:text-surface-300">الدور *</label>
              <select
                value={formData.role}
      ...
      {managingPermsFor && (
        <div className="mb-6 rounded-2xl border border-amber-200 bg-amber-50/30 p-5 dark:border-amber-800 dark:bg-amber-950/30">
          ...
          {userPermissions === undefined ? (
            ...
            <p className="mb-3 text-xs text-surface-500 dark:text-surface-400">لا توجد صلاحيات</p>
          ) : (
            <div className="mb-3 space-y-2">
              {userPermissions.map((perm) => (
                <div key={perm._id} className="flex items-center justify-between rounded-lg bg-white px-3 py-2 text-sm dark:bg-surface-800">
                  <span className="text-surface-700 dark:text-surface-200">{getMajorName(perm.majorId)}</span>
                  <button
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <option value="">اختر تخصصا</option>
      ...
      {users === undefined ? (
        ...
        <div className="rounded-2xl border border-surface-200 bg-white p-12 text-center dark:border-surface-700 dark:bg-surface-900">
          <p className="text-sm font-medium text-surface-700 dark:text-surface-200">لا يوجد مستخدمون</p>
        </div>
      ) : (
        <div className="space-y-3">
          {users.map((u) => (
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <h3 className="truncate text-sm font-semibold text-surface-900 dark:text-surface-50">{u.name}</h3>
                    ...
                    <p dir="ltr" className="text-xs text-surface-500 dark:text-surface-400">{u.email}</p>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                {u.role === "contributor" && (
                  <button
                    onClick={() => { setManagingPermsFor(u._id); setEditingId(null); setSelectedMajorId(""); }}
                ...
                <button
                  onClick={() => { handleEdit(u); setManagingPermsFor(null); }}
